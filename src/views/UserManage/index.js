...
import { fakeUserList, fakeSystemParam } from './fakeData';
...
import ErrorMessageBar from 'components/ErrorMessageBar'; // FluentUI
import { PrimaryButton, DefaultButton, SplitButton } from 'components/BaseButton';
import Pagination from 'components/Paginator/pagination'; // BasePaginator
...
const UserManage = () => {
  ...
  const addUser = (
    ...
    return createUser({
      ...
      .then(() => (`${t('add')}${t('enSpace')}${t('User')}${t('enSpace')}${t('success')}: ${username}`))
      .catch((err) => (`${t('add')}${t('enSpace')}${t('User')}${t('enSpace')}${t('fail')}: ${username} / ${err?.data?.message}`));
  };
  ...
  const addUserRecursively = (userInfos, index, results) => {
    ...
    if (index >= userInfos.length) {
      ...
      toast.success(<>{results.map(item => <div>{item}</div>)}</>, { autoClose: 10000 });
      getUserInfoList();
      return;
    } else {
  ...
  const getUserInfoList = useCallback(() => {
    setIsDataLoading(true);
    setUserInfoList(fakeUserList)
    setSystemParams(fakeSystemParam)
    Promise.all([
      getUserList(),
