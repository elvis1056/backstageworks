...
*/
const ViewTemplateModal = ({ isOpen, onClose, vgInfos, templateInfo }) => {
  ...
  useEffect(() => {
    if (virtualCluster && !isEmpty(vgInfos) && !isEmpty(taskRoles[selectedKey]) && templateInfo.jobConfig !== undefined) {
      ...
      const unitCountStr = !isEmpty(unitCount) && unitCount[selectedKey]
        ? `${unitCount[selectedKey].unitName} (${unitCount[selectedKey].gpuUnit} GPU, ${unitCount[selectedKey].cpuUnit} CPU, ${unitCount[selectedKey].memoryUnit} memory) * ${unitCount[selectedKey].count}`
        : ''
  ...
  return (
    ...
    >
      <div className={styles.container}>
        {
          (templateInfo.owner) || (templateInfo.description) &&
          <>
        ...
        <div className={styles.wrapper}>
          ...
          <div className={`${styles.wrapper__block} ${styles.marginTop10} ${styles.paddingBottom10} ${styles.wrapper__borderBottom}`}>
            ...
            <div className={styles.wrapper__block__item}>
              <div className={styles.block__item__50}>{selectedTaskRole !== undefined && selectedTaskRole.completion && selectedTaskRole.completion.minFailedTaskCount}</div>
              <div className={styles.block__item__50}>{selectedTaskRole !== undefined && selectedTaskRole.completion && selectedTaskRole.completion.minSucceededTaskCount}</div>
            </div>
          </div>
